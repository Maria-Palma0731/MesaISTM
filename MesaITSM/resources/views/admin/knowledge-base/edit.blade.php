    ...
    <div class="py-6">
        <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="category" id="category" required
                                ...
                                <option value="guias" {{ old('category', $article->category) == 'guias' ? 'selected' : '' }}>
                                    Guías y Tutoriales
                                </option>
                                <option value="faq" {{ old('category', $article->category) == 'faq' ? 'selected' : '' }}>
                                    Preguntas Frecuentes
                                </option>
                                <option value="tecnica" {{ old('category', $article->category) == 'tecnica' ? 'selected' : '' }}>
                                    Documentación Técnica
                                </option>
                                <option value="politicas" {{ old('category', $article->category) == 'politicas' ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <textarea name="summary" id="summary" rows="3"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                                placeholder="Breve descripción del artículo">{{ old('summary', $article->summary) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="content" id="content" rows="16" required
                                ...
                                placeholder="Escribe el contenido del artículo aquí...">{{ old('content', $article->content) }}</textarea>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                                <div>
                                    ...
                                    <span class="font-medium text-gray-900 ml-2">{{ $article->creator->name }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Fecha de creación:</span>
                                    <span class="font-medium text-gray-900 ml-2">{{ $article->created_at->format('d/m/Y H:i') }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Vistas totales:</span>
                                    <span class="font-medium text-gray-900 ml-2">{{ $article->views }}</span>
                                </div>
                                <div>
                                    <span class="text-gray-600">Última actualización:</span>
                                    <span class="font-medium text-gray-900 ml-2">{{ $article->updated_at->diffForHumans() }}</span>
                                </div>
                            </div>
                        </div>
                ...
                <form id="delete-form" action="{{ route('admin.knowledge-base.destroy', $article->id) }}" method="POST" class="hidden">
                    @csrf
                    @method('DELETE')
                </form>
            </div>
