                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody>
                                ...
                                @foreach($services as $service)
                                    @if($service->category_id !== $lastCategoryId)
                                        <tr class="bg-gray-50">
                                            <td colspan="9" class="px-6 py-2 text-sm font-medium text-gray-900">
                                                {{ $service->category->name }}
                                            </td>
                                        </tr>
                                        @php $lastCategoryId = $service->category_id; @endphp
                                    @endif

                                    <tr class="bg-white" data-id="{{ $service->id }}">
                                        ...
                                        <td class="px-6 py-4 text-sm font-medium text-right whitespace-nowrap">
                                            <a href="{{ route('admin.catalog.services.edit', $service) }}" 
                                                class="text-indigo-600 hover:text-indigo-900">
                                                Editar
                                            </a>
                                            <form action="{{ route('admin.catalog.services.destroy', $service) }}" 
                                                method="POST"
            ...
            if (table) {
                new Sortable(table, {
                    ...
                    onEnd: function() {
                        ...
                        fetch('{{ route("admin.catalog.services.reorder") }}', {
                            method: 'POST',
                            headers: {
                            ...
                            if (!response.ok) {
                                console.error('Error al reordenar servicios');
                            }
                        });
                    }
                });
